...
while key!=ord('q'):
  ...
  #ここでカメラモーターを制御したい
   ...
   if center_x*0.85 <= cpx <= center_x + center_x*0.15:
   ...
   #(右から)左へ向かせたい
   elif cpx < center_x*0.85: #0---144
      
      
      cpx_left = center_x*0.85 - cpx #cpx_leftはcpxとcenter*0.85の差
      mr = int( (cpx_left / center_x*0.85) * 60  ) #motor run  モーター出力
      
      #time.sleep(1)
      #csv.run(mr-20)

      for degree in range(mr-3, mr+10):
         dc = degree
         csv.run(dc)
         time.sleep(DT)
         if degree ==  mr+10 :
   ...
   #(左から)右へ向かせたい
   elif center_x + center_x*0.15 < cpx <= center_x*2: 
